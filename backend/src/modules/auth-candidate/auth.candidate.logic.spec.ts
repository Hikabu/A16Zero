...
describe('Auth Module Pure Logic', () => {
  describe('Register Schema Validation', () => {
    it('should fail if email and username are missing', () => {
      const result = registerSchema.safeParse({ password: 'Password123!', role: 'CANDIDATE' });
    ...
    it('should fail with weak password', () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.error.issues.some(e => e.message.includes('Password must contain'))).toBe(true);
      }
    });
