...
export class ScoringService {
   ...
   */
  score(data: GitHubRawData): AnalysisResult {
   ...
   */
  private computeCapabilities(signals: ExtractedSignals): AnalysisResult['capabilities'] {
    ...
    if (stackIdentity.length === 0) {
      ...
      const primaryWeights = LANGUAGE_CAPABILITY_WEIGHTS[primaryLang] || LANGUAGE_CAPABILITY_WEIGHTS['_unknown'];
  ...
  private computeOwnership(data: GitHubRawData): AnalysisResult['ownership'] {
    ...
    return {
      ...
      confidence: nonForkRepos.length > 5 ? 'high' : (nonForkRepos.length > 2 ? 'medium' : 'low'),
    };
  }
  ...
  private computeImpact(signals: ExtractedSignals, data: GitHubRawData): AnalysisResult['impact'] {
    ...
    return {
      ...
      confidence: activityLevel === 'high' ? 'high' : (activityLevel === 'medium' ? 'medium' : 'low'),
    };
  }
}
